...
export default function Home() {
  ...
  useDocumentTitle("Create Playlist - Musicelly");
      ...
      <main className="container" id="home">
        ...
        <div className="content">
          ...
          <div className="tracks">
            {tracks.map((track) => (
              <Track key={track.id} imageUrl={track.album.images[0].url} title={track.name} artist={track.artists[0].name} select={selectedTracksUri.includes(track.uri)} toggleSelect={() => toggleSelect(track)} />
            ))}
          </div>
        </div>
      </main>
